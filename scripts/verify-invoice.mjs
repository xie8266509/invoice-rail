...
const [invoiceId, rawRecipient, amount, symbol = "USDC"] = cliArgs;
if (!invoiceId || !rawRecipient || !amount || !(symbol in TOKENS)) {
  console.error("Usage: pnpm verify:invoice -- <invoice-id> <recipient> <amount> [USDC|EURC]");
  process.exit(1);
}
...
while (toBlock >= oldestBlock && !match) {
